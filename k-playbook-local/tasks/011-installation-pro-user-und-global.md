...
| 4 — Update-Hinweis über MCP | erledigt | 2026-09-26 | `upgrade.Watcher` in `serve` (Rolle Node): beim Start, dann alle 24 h, nach Fehler 1 h, eigene Goroutine, nur im Speicher; `whoami` hat `update` (Struktur wie `upgrade --check --json`, Sicht von `serve`), Textzeile „Update: …“; `node whoami` fragt direkt, `--json` gleich der MCP-Antwort; Konzept-Tabelle ergänzt; Tests mit gesteuerter Zeit und `httptest` |
| 5 — macOS in CI | erledigt | 2026-09-26 | Job `macos` (`macos-latest`, SHA-gepinnt, `GOTOOLCHAIN=local`): `check-toolchain`, `check`, `dist-host`, LaunchAgent mit `plutil -lint`, `install.sh` mit v0.1.1 (Tag aus der Weiterleitung, ohne API) in leerem `HOME`; gebaut und gelaufen, grün in 36259254294 (3. Versuch) und 36260980326, sonst einzig `TestBackgroundSync` rot (Befund `material/befunde/ci-macos.md`). Auf Wunsch des Nutzers seit 2026-09-26 mit `if: false` abgeschaltet, Definition bleibt; wieder einschalten steht in `docs/fortschritt.md` |
| 6 — Doku | erledigt | 2026-09-26 | `docs/installation.md` neu (Überblick, pro User, global von Hand, Upgrade und JSON-Felder, ohne systemd, Automatisierung mit Ansible-Beispiel; `--syntax-check` grün); README (Installation, Upgrade, serve, Bauen), Begriffe (config, init, status, upgrade, system installation, linger, whoami), Konzept (Stand, Installation und Betrieb: `/etc/kephalaion` → `kephalaion` 0755/config 0644, Label `io.github.kephalaion`, JSON-Felder, „vorläufig“ und „nicht gebaut“ ersetzt), Projektregeln (Pakete `service`, `upgrade`, CI macOS), Fortschritt |
| 7 — Durchlauf | erledigt | 2026-09-26 | pro User echt: `service install` brach bei laufendem `serve` von Hand ab (PID 75501, nach Neustart durch eine andere Sitzung; per SIGTERM beendet), danach eingerichtet, `systemd-analyze --user verify` ok, Neustart über `make dev-install` (neue PID, neues `ActiveEnterTimestamp`, neues Binary), `uninstall` und wieder `install`; Dienst läuft mit dem zuletzt gebauten Binary, Linger aus. Global im Container nach der Doku (Rechte, `serve` als `kephalaion`, zweiter User: `status` Exit 0, `whoami`/`read` über Loopback, `upgrade --check` Weg `admin`, `init`/`service install` brechen ab, zwei Arten erkannt), System-Unit mit `systemd-analyze verify`. Ansible gegen zweiten Container: v0.1.1 mit Prüfsumme, dann lokales Binary (nur im Durchlauf), `kephalaion_manage_service=false`, zweiter Lauf ohne Änderung. macOS: Job gebaut und gelaufen, grün in 36259254294 und 36260980326 (samt `plutil -lint` und `install.sh`), sonst einzig `TestBackgroundSync` rot (Läufe 36260080975, 36260519391); auf Wunsch des Nutzers abgeschaltet (`if: false`) |
