...
| 5 — macOS in CI | erledigt | 2026-09-26 | Job `macos` (`macos-latest`, SHA-gepinnt, `GOTOOLCHAIN=local`): `check-toolchain`, `check`, `dist-host`, LaunchAgent mit `plutil -lint`, `install.sh` mit v0.1.1 (Tag aus der Weiterleitung, ohne API) in leerem `HOME`; Lauf 36259254294 grün (Linux und macOS), macOS erst im 3. Versuch — `TestBackgroundSync` (Befund `material/befunde/ci-macos.md`) |
...
| 7 — Durchlauf | erledigt | 2026-09-26 | pro User echt: `service install` brach bei laufendem `serve` von Hand ab (PID 75501, nach Neustart durch eine andere Sitzung; per SIGTERM beendet), danach eingerichtet, `systemd-analyze --user verify` ok, Neustart über `make dev-install` (neue PID, neues `ActiveEnterTimestamp`, neues Binary), `uninstall` und wieder `install`; Dienst läuft mit dem zuletzt gebauten Binary, Linger aus. Global im Container nach der Doku (Rechte, `serve` als `kephalaion`, zweiter User: `status` Exit 0, `whoami`/`read` über Loopback, `upgrade --check` Weg `admin`, `init`/`service install` brechen ab, zwei Arten erkannt), System-Unit mit `systemd-analyze verify`. Ansible gegen zweiten Container: v0.1.1 mit Prüfsumme, dann lokales Binary (nur im Durchlauf), `kephalaion_manage_service=false`, zweiter Lauf ohne Änderung. macOS-Job grün in den Läufen 36259254294 und 36260980326 (HEAD 76b70e0, samt `plutil -lint` und `install.sh`); die übrigen Läufe dort nur an `TestBackgroundSync` rot (Befund; bis Task 013 ignoriert) |
